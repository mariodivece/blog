...
What is important, is that you understand that *IP* defines some basic aspects of communications and networking. It is **NOT** TCP/IP so please drop the term TCP for now because TCP is a protocol that is part of the *suite* of protcols that build on top of *IP*. Let me repeat that: Protocols such as *TCP* and *UDP* are part of a *suite* of protocols build on top of the *IP* standard.

So, let's cut to the chase: what standard aspects are defined by the *Internet Protocol*, or *IP*? Many, in fact. But you just need tu understand a few of them.

 - Addresses: It defines a way to represent network addresses
 - Packets: It defines a transmission unit contining information about the source, destination and contents of the messages between senders and receivers.
 ...
 - Ports: In addition to identifying source and destination computers, ports are used to identify 

In contrast, it does **not** define:
 - The order in which packets are sent and received.
 - Receiving or sending acknoledgementas that packets were received at the destination.
