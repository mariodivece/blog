...
Recall we stated Gateways or Routers are computers that understand how to make requests and receive responses from and to other machines on behalf of the clients behind them. So, we have several hosts behind a Router that communicate to the rest of the Internet by asking the router to forward packets to a remote destination. But before we explore how hosts behind a router reach destinations on the vast Internet, we will explore how machines communicate on a *Local Network*.
