...
- **Ports**: Surprisingly enough, IP does *not* define ports. We use addresses to identify hosts, and we use ports to identify applications within hosts. This allows different programs running in the same host to communicate with other hosts simultaneously. Ports are a notion that belongs to the realm of Transport level protocols (such as *TCP* and *UDP*) and not to IP.
 
In other words, *IP* defines **messaging** between hosts, simplifying potentially complex network topologies by  *hiding* or *abstracting* the physical details away. You should now think about how the OSI model is flexible enough and how IP is an  **independent** protocol connecting systems together. In the future we could have a different protocol replace IP (in fact its is happening with *IPv6*, and we will learn more about this later) and still not be required to change -- at least not entirely -- the way Transport level protocols or Application level protocols work.
...
#### Knowledge Check
 ...
 1. What are the fundamental differences between the PDUs used by the Link Layer and the Network Layer
