...
As you can see from the table above, it would be easy to map back and forth between layer models. So don't get into big debates about which model is correct. The value in a layered model lies within the ability to decouple, to the maximium possible extent the various protocols at the different layers they operate. This allows for new protocols and existing protocols to evolve with little to no regard for protocols in layers below them. Check out this super interesting (partial) list of [popular protocols at the different OSI model layers](https://en.wikipedia.org/wiki/List_of_network_protocols_(OSI_model)).
...
In other words, *IP* defines **how to create connections** between hosts, simplifying potentially complex network topologies by  *hiding* or *abstracting* the physical details away. You should now think about how the OSI model is flexible enough and how IP is an  **independent** protocol in system connections. In the future we could have a different protocol replace IP (in fact its is happening with *IPv6*, and we will learn more about this later) and still not be required to change -- at leats not entirealy -- the way Transport level protocols or applications work.
